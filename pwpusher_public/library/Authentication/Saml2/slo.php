...
use saml\src\Saml2\IdPMetadataParser;
...
$samlSettings = new saml\src\Saml2\Settings($settingsInfoArray);
...
if (isset($_SESSION['IdPSessionIndex']) && !empty($_SESSION['IdPSessionIndex'])) {
    $logoutRequest = new saml\src\Saml2\LogoutRequest($samlSettings, null, $_SESSION['IdPSessionIndex']);
} else {
    $logoutRequest = new saml\src\Saml2\LogoutRequest($samlSettings);
}
...
$url = saml\src\Saml2\Utils::redirect($sloUrl, $parameters, true);
